<?php
    ...
    $statement = $pdo->prepare("SELECT ST.student_number AS student_number,ST.name AS NAME,AC.password AS password,IFNULL(SUB.DAT,'活動記録なし') AS date
                                FROM students ST
                                INNER JOIN accounts AC
                                ON ST.teacher = ?
                                AND ST.student_number = AC.id
                                LEFT OUTER JOIN (SELECT DATE_FORMAT(report_date,'%Y年%m月%d日')AS DAT,poster FROM reports GROUP BY poster ORDER BY poster) SUB 
                                ON AC.id = SUB.poster;");
